...
## Summary
The purpose of this project is to use Deep Learning Neural Network to build an algorithm that classifies a set X-rays images belonging to pediatric patients to help determine if whether or not pneumonia is present. The Neural Network chosen was the Convolutional Neural Network (CNN) as it is one of the preferred for image processing.
